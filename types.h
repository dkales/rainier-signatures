...
typedef std::array<uint8_t, SALT_SIZE> salt_t;

typedef std::pair<std::vector<std::vector<uint8_t>>, size_t> reveal_list_t;

typedef std::array<uint8_t, 16> aes_block_t;

typedef std::pair<std::vector<uint8_t>, std::vector<uint8_t>> keypair_t;

template <typename GF> struct repetition_proof_t {
